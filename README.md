...
> **Note:** Cloud deployments require a remote Ollama instance or switching to a cloud LLM API (OpenAI-compatible).

### Hugging Face Spaces

1. Create a new Space (Streamlit template)
2. Upload `frontend/app.py` as `app.py`
3. Set `BACKEND_URL` env var to your deployed backend URL
...
- Reduce context: lower `MAX_SEARCH_RESULTS` in `.env`

**ChromaDB errors:**
```bash
rm -rf backend/chroma_db  # Reset vector store
```

**Port already in use:**
```bash
lsof -ti:8000 | xargs kill  # Kill backend port
lsof -ti:8501 | xargs kill  # Kill frontend port
```
